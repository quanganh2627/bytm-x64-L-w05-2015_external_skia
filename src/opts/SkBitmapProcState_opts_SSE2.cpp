...
// below is the implementation of S32_opaque_D32_filter_DX_SSE2 and SkPixel32ToPixel16
// see the definition in section of "D16 functions for SRC == 8888" in SkBitmapProcState.cpp
void S32_D16_filter_DX_SSE2(const SkBitmapProcState& s,
    ...
    do {
        ...
        // below is much faster than the above for the legacy benchmark--Softweg. Maybe it can still be tuned later
...
void S32_opaque_D32_filter_DXDY_SSE2(const SkBitmapProcState& s,
   ...
   if (count >= 4) {
       while (((size_t)xy & 0x0F) != 0)
       {
           ...
           Filter_32_opaque_portable(subX, subY,
                       (row0[x0]),
                       (row0[x1]),
                       (row1[x0]),
                       (row1[x1]),
      ...
      while (((size_t)xy & 0x0F) != 0)
       {
           ...
           Filter_32_alpha_portable(subX, subY,
                       (row0[x0]),
                       (row0[x1]),
                       (row1[x0]),
                       (row1[x1]),
